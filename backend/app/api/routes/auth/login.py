...
@router.post("/login/requires-otp", status_code=status.HTTP_200_OK)
async def request_login_otp(
    login_data: LoginRequestSchema, db: AsyncSession = Depends(get_session)
):
    try:
        user = await user_auth_service.get_user_by_email(login_data.email, db)

        if user:
            await user_auth_service.check_user_lockout(user, db)

            if not await user_auth_service.verify_user_password(
                login_data.password, user.hashed_password
            ):
                await user_auth_service.increment_failed_login_attempts(user, db)
                remaining_attempts = (
            ...
            await user_auth_service.reset_user_state(
                user, db, clear_otp=True, log_action=True
            )

            await user_auth_service.generate_and_save_otp(user, db)
...
async def verify_login_otp(
    ...
    try:
        user = await user_auth_service.verify_login_otp(
            verify_data.email, verify_data.otp, db
        )
        await user_auth_service.reset_user_state(
            user, db, clear_otp=True, log_action=True
        )
